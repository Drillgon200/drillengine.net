...
// iso, member-body, us, rsadsi, pkcs, pkcs-9, id-ExtensionReq
constexpr U32 ASN1_OID_EXTENSION_REQUEST[]{ 1, 2, 840, 113549, 1, 9, 14 };
...
constexpr U8 ASN1_RAW_OID_SUBJECT_ALT_NAME[]{ 2 * 40 + 5, 29, 17 };
		...
		if (firstByte & 0b10000000) {
			...
			if (exponentFormat == 0b11) {
				...
				if (exponentLength + 4 > 9) {
					return signBit ? -F32_INF : F32_INF;
				}
